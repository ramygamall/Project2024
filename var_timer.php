...
<head>
    ...
    <style>
        /* إضافة الأنماط CSS الخاصة بالتايمر */
        .container {
            text-align: center;
        ...
        .timer {
            font-size: 10em; /* Reduced font size for timer */
            font-weight: bold;
            color: #333;
            margin-bottom: 20px;
    ...
    <ul class="menu">
        ...
        <li class="dropdown">
            ...
            <ul id="varTimerDropdown" class="dropdown-menu <?php echo ($current_page == 'var_timer.php') ? 'show' : ''; ?>">
                <li><a href="add_referee.php" class="<?php echo ($current_page == 'add_referee.php') ? 'active' : ''; ?>"><i class="fas fa-plus-circle"></i> Add Referee</a></li>
                <li><a href="add_group.php" class="<?php echo ($current_page == 'add_group.php') ? 'active' : ''; ?>"><i class="fas fa-users"></i> Add Group</a></li>
                <li><a href="edit_referee.php" class="<?php echo ($current_page == 'edit_referee.php') ? 'active' : ''; ?>"><i class="fas fa-edit"></i> Edit Referee</a></li>
                <li><a href="var_timer.php" class="<?php echo ($current_page == 'var_timer.php') ? 'active' : ''; ?>"><i class="fas fa-clock"></i> Timer Setting</a></li>
            </ul>
        </li>
...
<div class="popup" id="popup">
    <div class="popup-content">
        <div class="popup-header">Select Referee</div>
        <div class="dropdown-container">
            <select id="refereeSelect" class="js-example-basic-single">
                <option value="Peter Waweru Kamaku">Peter Waweru Kamaku</option>
                <option value="TOM Abongile">TOM Abongile</option>
                <!-- Add more options as needed -->
            </select>
        </div>
        <div class="popup-footer">
            <button class="btn-red" onclick="closePopup()">Cancel</button>
            <button class="btn-blue" onclick="addRecord()">Add</button>
        </div>
    </div>
</div>
...
<script>
    ...
    function closePopup() {
        ...
        // Reset the add button text back to "Add" after closing the popup
    ...
    function addRecord() {
        ...
        let dateTimeString = currentDate.toLocaleDateString() + " " + currentDate.toLocaleTimeString();
        recordList.push({
            date: currentDate,
            time: formatTime(Math.floor(totalTime / (1000 * 60))) + ":" + formatTime(Math.floor((totalTime / 1000) % 60)),
            referee: refereeName
        });
        renderRecords(); // Update the table with the new record
        closePopup(); // Close the popup after adding the record
    }
    ...
    function renderRecords() {
        ...
        recordList.forEach((record, index) => {
            ...
            dateCell.textContent = record.date.toLocaleDateString(); // Display date
            timeCell.textContent = record.time; // Display formatted time
            refereeCell.textContent = record.referee; // Display referee name
    ...
    function editRecord(index) {
        // Open the popup with current referee name for editing
        openPopup();
        document.getElementById("refereeSelect").value = recordList[index].referee;

        // Replace the add button with an update button in the popup
        let addButton = document.querySelector(".popup-footer .btn-blue");
    ...
    function updateRecord(index) {
        ...
        renderRecords(); // Update the table with the edited record
        closePopup(); // Close the popup after updating the record
    }

    function deleteRecord(index) {
        recordList.splice(index, 1); // Remove the record from the array
